...
export default function About({ data }: AboutProps) {
  ...
  const getBadgePositions = () => {
    if (isMobile) {
      return {
        fullStackDev: "top-4 left-1/2 transform -translate-x-1/2",
        mernStack: "right-2 top-1/2 transform -translate-y-1/2",
        reactNative: "bottom-4 left-1/2 transform -translate-x-1/2",
        cloudEngineer: "left-2 top-1/2 transform -translate-y-1/2",
      }
    }
    if (isTablet) {
      return {
        fullStackDev: "top-6 left-1/2 transform -translate-x-1/2",
        mernStack: "right-3 top-1/2 transform -translate-y-1/2",
        reactNative: "bottom-6 left-1/2 transform -translate-x-1/2",
        cloudEngineer: "left-3 top-1/2 transform -translate-y-1/2",
      }
    }
    return {
      fullStackDev: "top-8 left-1/2 transform -translate-x-1/2",
      mernStack: "right-4 top-1/2 transform -translate-y-1/2",
      reactNative: "bottom-8 left-1/2 transform -translate-x-1/2",
      cloudEngineer: "left-4 top-1/2 transform -translate-y-1/2",
    }
  }
          ...
          >
            ...
            <div className="grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
              ...
              <div className="space-y-4 sm:space-y-6 order-2 lg:order-1">
                <p className="text-base sm:text-lg text-gray-300 leading-relaxed text-justify">{data.description}</p>

                <div className="space-y-3 sm:space-y-4">
                  <h3 className="text-xl sm:text-2xl font-semibold text-white">Key Strengths</h3>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
                    {data.highlights.slice(0, 10).map((highlight, index) => (
                      <div
                        key={index}
              ...
              <div className={`relative flex justify-center items-center ${getContainerHeight()} order-1 lg:order-2`}>
                ...
                >
                  ...
                  >
                    <Image
                      ...
                      alt="Abhisekh Nayek - Full-Stack Developer"
                      layout="fill" // Use fill to make image responsive within parent
                ...
                {/* Full-Stack Dev - Top */}
                <motion.div
                  className={`absolute ${badgePositions.fullStackDev} z-20`}
                  animate={{
                    y: [0, isMobile ? -5 : -10, 0],
                    opacity: [0.8, 1, 0.8],
                  }}
                ...
                {/* MERN Stack - Right */}
                <motion.div
                  className={`absolute ${badgePositions.mernStack} z-20`}
                  animate={{
                    x: [0, isMobile ? 5 : 10, 0],
                    opacity: [0.8, 1, 0.8],
                  }}
                ...
                {/* React Native - Bottom */}
                <motion.div
                  className={`absolute ${badgePositions.reactNative} z-20`}
                  animate={{
                    y: [0, isMobile ? 5 : 10, 0],
                    opacity: [0.8, 1, 0.8],
                  }}
                ...
                {/* Cloud Engineer - Left */}
                <motion.div
                  className={`absolute ${badgePositions.cloudEngineer} z-20`}
                  animate={{
                    x: [0, isMobile ? -5 : -10, 0],
                    opacity: [0.8, 1, 0.8],
                  }}
